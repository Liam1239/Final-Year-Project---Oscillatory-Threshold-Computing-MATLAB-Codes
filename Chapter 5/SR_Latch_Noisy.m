Tmax = 2500; % The time period the simulation will be carried out over. 
...
input_current = 1; % This is the external input current applied to the two output oscillators. 
...
% Holds the inputs to the oscillators in the order I_1, I_2, O_1, O_2 for each of the rows. 
Oscillator_Inputs = [0 0 0 input_current % Start by providing an input just to O_2 (to make it oscillate). 
                     0 0 input_current input_current % Then provide the input to O_1 as well (shouldn't yet begin to oscillate, as it should be inhibited by O_2). 
                     1 0 input_current input_current % Then an input is provided to I_1 (set the latch).
                     0 0 input_current input_current % See the continuing output. 
                     0 1 input_current input_current % Then reset the latch. 
                     0 0 input_current input_current]; % Then see the outputs.

% Same Purpose as above, just reversing the operations in comparison to what they should be. 
Oscillator_Inputs2 = [0 0 input_current 0 
                     0 0 input_current input_current  
                     0 1 input_current input_current 
                     0 0 input_current input_current  
                     1 0 input_current input_current  
                     0 0 input_current input_current]; 
...
for looper1 = 1:size(Oscillator_Inputs,1) % Loop through all of the inputs in the Oscillator_Inputs matrix. 
    ...
    for looper2 = 1:Number_Of_Iterations - 1 % Need to go to - 1, otherwise errors will occur.
        ...
        % EULER METHOD: X(T + 1) = X(T) + (f(X(T)) * Timestep)
...
figure(name = 'Set-Reset (SR) Latch, Noisy') % Naming the flicker.
...
ylim([-1 1.5]); % Limits for the Y-Axis.
...
set(get(gca,'ylabel'),'rotation',0); % Making the y-axis horizontal.
...
sgt = sgtitle('\textbf{Oscillatory Threshold Computing, Set-Reset (SR) Latch Output - Sustained Inputs / Inpulses, With Noise $\sigma$ = 0.1}', fontsize = 18, Interpreter='latex');% Title of the graph. 
