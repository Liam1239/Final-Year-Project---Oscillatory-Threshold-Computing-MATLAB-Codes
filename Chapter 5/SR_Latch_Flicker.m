tmax=1; % We change the time for solving the equation to just 1 in this version. We do this because we significantly change the number of times we solve the system. (see inputs matrix I1 & I2 for reasoning).
tspan=[0 tmax]; % Timespan for solving the system. 

e1 = 0.15; % just needs to provide enough to overcome the strength of x_1 (i1). It works with Input_current to do so. 
i1 = -0.65;
input_current = 1;
...
for loop1=1:size(Oscillator_Inputs,1) % Loops through 4 times - This represents our 4 sets of inputs. (0,0),(0,1),(1,0) & (1,1). 
    [t,y]=ode45(@FN,tspan,yinit,[],[a b c],Oscillator_Inputs(loop1,:)',mat, m,p); 
...
for loop1=1:size(Oscillator_Inputs2,1) % Loops through 4 times - This represents our 4 sets of inputs. (0,0),(0,1),(1,0) & (1,1). 
    [t,y]=ode45(@FN,tspan,yinit,[],[a b c],Oscillator_Inputs2(loop1,:)',mat, m,p); 
    % tspan is just simply the set of time that you are going to be solving over. 
    % yinit is your initial values to solve for. 
    % [] 
    % [a b c]. a = Threshold voltage for spiking / b = oscillatory frequency (ensures that blocking mechanism doesn't just continuously grow. If we have that b is high then the oscillatory frequency is high.  c = critical point location / growth rate of blocking mech. 
...
figure(name = 'Set-Reset (SR) Latch Flicker');
t = tiledlayout(4,2);
t.Padding = 'compact';
...
sgt = sgtitle('\textbf{Oscillatory Threshold Computing, Set-Reset (SR) Latch Output - Small Impulses, Without Noise}', fontsize = 18, Interpreter='latex'); % Title of the graph. 
